- [Basic chapter 2 - MySQL环境搭建](#basic-chapter-2---mysql环境搭建)
  - [1. MySQL的卸载](#1-mysql的卸载)
    - [步骤1：停止MySQL服务](#步骤1停止mysql服务)
    - [步骤2：软件卸载](#步骤2软件卸载)
    - [步骤3：残余文件的清理](#步骤3残余文件的清理)
    - [步骤4：清理注册表](#步骤4清理注册表)
    - [步骤5：删除环境变量配置](#步骤5删除环境变量配置)
  - [2. MySQL的下载、安装、配置](#2-mysql的下载安装配置)
    - [MySQL的4大版本](#mysql的4大版本)
    - [软件的下载](#软件的下载)
    - [MySQL8.0 版本的安装](#mysql80-版本的安装)
    - [配置MySQL8.0](#配置mysql80)
    - [配置MySQL8.0 环境变量](#配置mysql80-环境变量)
    - [MySQL5.7 版本的安装、配置](#mysql57-版本的安装配置)
    - [安装失败问题](#安装失败问题)
  - [3. MySQL的登录](#3-mysql的登录)
    - [服务的启动与停止](#服务的启动与停止)
    - [自带客户端的登录与退出](#自带客户端的登录与退出)
  - [4. MySQL演示使用](#4-mysql演示使用)
    - [MySQL的使用演示](#mysql的使用演示)
      - [1、查看所有的数据库](#1查看所有的数据库)
      - [2、创建自己的数据库](#2创建自己的数据库)
      - [3、使用自己的数据库](#3使用自己的数据库)
      - [4、查看某个库的所有表格](#4查看某个库的所有表格)
      - [5、创建新的表格](#5创建新的表格)
      - [6、查看一个表的数据](#6查看一个表的数据)
      - [7、添加一条记录](#7添加一条记录)
      - [8、查看表的创建信息](#8查看表的创建信息)
      - [9、查看数据库的创建信息](#9查看数据库的创建信息)
      - [10、删除表格](#10删除表格)
      - [11、删除数据库](#11删除数据库)
    - [MySQL的编码设置](#mysql的编码设置)
      - [MySQL5.7中](#mysql57中)
      - [MySQL8.0中](#mysql80中)
  - [5. MySQL图形化管理工具](#5-mysql图形化管理工具)
  - [6. MySQL目录结构与源码](#6-mysql目录结构与源码)
    - [主要目录结构](#主要目录结构)
    - [MySQL 源代码获取](#mysql-源代码获取)
  - [7. 常见问题的解决](#7-常见问题的解决)
  - [面试题](#面试题)
